...
// Resources are copied next to the executable; outputs stay in the directory the game was launched from.
string launchDir = Directory.GetCurrentDirectory();
...
if (benchmark || profile)
{
    ...
    profiler.EnableCsvLog(Path.Combine(launchDir, "profile.csv"));
}
...
if (profiler.Enabled)
{
    ...
    File.WriteAllText(Path.Combine(launchDir, "profile_report.txt"), report);
    profiler.Shutdown();
}
